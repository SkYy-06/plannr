...
import { UnsplashModal } from "@/components/unsplash-image-picker";
import { Label } from "@/components/ui/label";
import { Badge } from "@/components/ui/badge";
import { CalendarIcon, Crown, Sparkles, Loader2 } from "lucide-react";
            ...
            <div className="flex gap-2 flex-wrap">
              {colorPresets.map((color) => (
                <button
                  key={color}
                  type="button"
                  className={`w-10 h-10 rounded-full border-2 transition-all ${!hasPro && color !== "#1e3a8a" ? "opacity-4 cursor-not-allowed" : "hover:scale-110"}`}
                  style={{
              ...
              {!hasPro && (
                ...
                >
                  <Sparkles className="w-5 h5 text-purple-400" />
                </button>
              )}
            </div>
            {!hasPro && (
